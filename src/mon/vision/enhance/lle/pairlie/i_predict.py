    ...
    with mon.create_progress_bar() as pbar:
        for i, datapoint in pbar.track(
            ...
            if save_image:
                ...
                mon.save_image(I, output_path)

            # Save Debug
            if save_debug:
                debug_dir = mon.parse_output_dir(save_dir, data_name, mon.SAVE_DEBUG_DIR, image_path, keep_subdirs, save_nearby)
                debug_dir.mkdir(parents=True, exist_ok=True)
                mon.save_image(L, debug_dir / f"{image_path.stem}_L{mon.SAVE_IMAGE_EXT}")
                mon.save_image(R, debug_dir / f"{image_path.stem}_R{mon.SAVE_IMAGE_EXT}")
                mon.save_image(D, debug_dir / f"{image_path.stem}_D{mon.SAVE_IMAGE_EXT}")
