...
import libs.src.tester as tester
# sys.path.append("/home2/saurabh.saini/WORK/CODES/RRR")
import libs.src.trainer as trainer
...
def chk_args(config):
    ...
    if config.mode not in set_modes:
        sys.exit("ERROR: Incorrect mode (should be : " + str(set_modes))
    config.device = config.device.lower()
    if config.device not in set_devices:
        sys.exit("ERROR: Incorrect device type (should be :" + str(set_devices))
    config.dataset  = config.dataset.lower()
    if config.dataset not in set_datasets:
        sys.exit("ERROR: Dataset not found (should be :" + str(set_datasets))
    if config.resume:
        if not (os.path.exists(config.p_model)):
            sys.exit("ERROR: Model_path incorrect.")
    if config.mode is "train":
        if not (os.path.exists(config.p_trainDir)):
            sys.exit("ERROR: Dataset_path incorrect.")
        if not (os.path.exists(config.p_trainList)):
            sys.exit("ERROR: Train_inList_path incorrect.")
        if not (os.path.exists(config.p_trainGtDir)):
            sys.exit("ERROR: Train_gt_path incorrect.")
    if config.mode is "test":
        if not (os.path.exists(config.p_model)):
            sys.exit("ERROR: Model_path incorrect.")
        if not (os.path.exists(config.p_testDir)):
            sys.exit("ERROR: TestDir path incorrect.")
        if (not config.p_testList) and (not os.path.exists(config.p_testList)):
            sys.exit("ERROR: Train_inList_path incorrect.")
        if (not config.p_testGtDir) and (not os.path.exists(config.p_testGtDir)):
            sys.exit("ERROR: Test_gt_path incorrect.")
    if not config.p_resDir:
        os.makedirs(config.p_resDir, exist_ok=True)
    if not (config.lr > 0 and isinstance(config.lr, float)):
        sys.exit("ERROR: Incorrect learning rate value.")
    if not (config.epochs > 0) and isinstance(config.epochs, int):
        sys.exit("ERROR: Incorrect pre_training epochs value.")
    if not (config.lr_step > 0 and isinstance(config.lr_step, int)):
        sys.exit("ERROR: Incorrect learning step value.")
    if not (config.lr_decay > 0 and isinstance(config.lr_decay, float)):
        sys.exit("ERROR: Incorrect learning decay value.")
    if not (config.batch_size > 0 and isinstance(config.batch_size, int)):
        sys.exit("ERROR: Incorrect batch_size value.")
