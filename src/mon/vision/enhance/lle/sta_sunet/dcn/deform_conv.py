        ...
        if not grad_output.is_cuda:
            ...
            if ctx.needs_input_grad[0] or ctx.needs_input_grad[1]:
                ...
                deform_conv_ext.deform_conv_backward_input(
                    input, offset, grad_output, grad_input, grad_offset, weight,
                    ctx.bufs_[0], weight.size(3), weight.size(2), ctx.stride[1],
                    ctx.stride[0], ctx.padding[1], ctx.padding[0], ctx.dilation[1],
                    ctx.dilation[0], ctx.groups, ctx.deformable_groups, cur_im2col_step
                )
...
class ModulatedDeformConvFunction(Function):
    ...
    def forward(ctx, input, offset, mask, weight, bias=None, stride=1, padding=0, dilation=1,
        ...
        deform_conv_ext.modulated_deform_conv_forward(
            ...
            weight.shape[2], weight.shape[3], ctx.stride, ctx.stride, ctx.padding,
            ctx.padding, ctx.dilation, ctx.dilation, ctx.groups, ctx.deformable_groups,
            ctx.with_bias
        )
        return output
    ...
    def backward(ctx, grad_output):
        ...
        deform_conv_ext.modulated_deform_conv_backward(
            input, weight, bias, ctx._bufs[0], offset, mask, ctx._bufs[1], grad_input,
            grad_weight, grad_bias, grad_offset, grad_mask, grad_output, weight.shape[2],
