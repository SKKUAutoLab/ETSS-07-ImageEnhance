...
from torchkit.core.runner.utils import get_next_version
...
class CheckpointCallback(Callback):
	...
	it can be retrieved at a later point in time and continued. The process
	...
	Attributes:
		checkpoint_dir (str, Path, optional):
			Directory to save the checkpoints. The checkpoints will be save to
		...
		last_global_step_saved (int):
			The last training step the checkpoint was saved. Default: `-1`.
		last_time_checked (float, optional):
			The last time the checkpoint was saved. Default: `None`.
		best_score (float, optional):
			The best monitored score. Default: `None`.
		current_score (float, optional):
			The current monitored score. Default: `None`.
		best_checkpoint_path (str, optional):
			The best checkpoint path. Default: `None`.
		last_checkpoint_path (str, optional):
			The last checkpoint path. Default: `None`.
	"""
	...
	def init_checkpoint_dir(
		...
		rank_zero_warn(f"Checkpoint directory at: {self.checkpoint_dir}.")
	...
	def init_monitor_mode(self, monitor: Optional[str], mode: str):
		...
		if monitor is None:
			monitor = "loss"
		if "train" not in monitor or "val" not in monitor:
			if (
				self.every_n_train_steps and
				(self.every_n_train_steps >= 1 or
	...
	def init_triggers(
		...
		if (
			every_n_train_steps is None and
			every_n_epochs is None and
			train_time_interval is None
		):
			every_n_epochs      = 1
			every_n_train_steps = 0
			logger.debug("Both `every_n_train_steps` and `every_n_epochs` "
	...
	def on_train_batch_end(
		...
		skip_batch = (self.every_n_train_steps < 1 or
					  ((step + 1) % self.every_n_train_steps != 0))
	...
	def on_train_epoch_end(
		...
		if (
			not self._should_skip_saving_checkpoint(trainer=trainer) and
			self.save_on_train_epoch_end and
			self.every_n_epochs > 0 and
	...
	def on_load_checkpoint(
		...
		trainer.fit_loop.current_epoch = \
			callback_state.get("epoch", 0)
		trainer.fit_loop.global_step = \
			callback_state.get("global_step", 0)
		self.best_score = \
			callback_state.get("best_score", self.MODE_DICT[self.mode])
		self.current_score = \
			callback_state.get("current_score", self.MODE_DICT[self.mode])
		self.best_checkpoint_path = \
			callback_state.get("best_checkpoint_path", "")
		self.last_checkpoint_path = \
			callback_state.get("last_checkpoint_path", "")
		...
		print(f"best_score: {self.best_score}, "
			  f"best_checkpoint_path: {self.best_checkpoint_path}")
		print(f"current_score: {self.current_score}, "
			  f"last_checkpoint_path: {self.last_checkpoint_path}")
	...
	def manual_load_checkpoint(self, checkpoint: Union[str, Path]):
		...
		if is_torch_saved_file(path=checkpoint):
			...
			self.best_score = \
				checkpoint.get("best_score", self.MODE_DICT[self.mode])
			self.current_score = \
				checkpoint.get("current_score", self.MODE_DICT[self.mode])
			self.best_checkpoint_path = \
				checkpoint.get("best_checkpoint_path", "")
			self.last_checkpoint_path = \
				checkpoint.get("last_checkpoint_path", "")
	...
	def _save_checkpoint(self, trainer: "pl.Trainer"):
		...
		self._save_best_checkpoint(trainer=trainer,
								   monitor_candidates=monitor_candidates)
		# Mode 2: Save last checkpoint
		self._save_last_checkpoint(trainer=trainer,
								   monitor_candidates=monitor_candidates)
	...
	def _save_best_checkpoint(
		self,
		trainer           : "pl.Trainer",
		monitor_candidates: dict[str, Union[int, float, torch.Tensor]]
	):
		...
		Args:
			...
			monitor_candidates (dict):
				The dictionary of all monitored metrics.
		"""
		...
		if (
			(current is None) or
			(isinstance(current, torch.Tensor) and torch.isnan(current))
		):
		...
		sorted_dict = dict(sorted(ckpt_dict.items(), key=lambda x: x[1],
								  reverse=reverse))
		best_path, best_score = list(sorted_dict.items())[0]
		
		# NOTE: Save
		if (
			self.best_checkpoint_path and
			self.best_checkpoint_path != best_path and
		...
		if best_path != self.best_checkpoint_path:
			...
			if self.verbose:
				...
				rank_zero_info(f"Epoch {epoch:d}, global step {step:d}: "
							   f"best monitor {current}.")
	...
	def _save_last_checkpoint(
		...
		# NOTE: Save
		if (
			self.last_checkpoint_path and
			self.last_checkpoint_path != filepath and
			trainer.should_rank_save_checkpoint
		):
			self._del_model(trainer=trainer,
	...
	def _del_model(self, trainer: "pl.Trainer", filepath: str):
		...
		Args:
			trainer (pl.Trainer):
				The `Trainer` object.
			filepath (str):
				The checkpoint path to delete.
		"""
		if (
			trainer.should_rank_save_checkpoint and
			os.path.exists(path=filepath)
		):
			os.remove(path=filepath)
			logger.debug(f"Removed checkpoint: {filepath}")
	...
	def _is_valid_monitor_key(
		self, metrics: dict[str, Union[int, float, torch.Tensor]]
	) -> bool:
		"""Check if model's metrics has `monitor` key.
		
		Args:
			metrics (dict):
				The metrics defined in the model.
	...
	def _resolve_checkpoint_dir(self, trainer: "pl.Trainer"):
		"""Determines model checkpoint save directory at runtime. References
		...
		The base path gets extended with logger name and version (if these are
		...
		if trainer.logger is not None:
			if trainer.weights_save_path != trainer.default_root_dir:
				# The user has changed `weights_save_path`, it overrides
				# anything
				checkpoint_dir = trainer.weights_save_path
			else:
				checkpoint_dir = (trainer.logger.save_dir or
	...
	def _format_checkpoint_path(
		...
		Args:
			...
			postfix (str, optional):
				The name postfix. One of: [`best`, `last`]. Default: `None`.
			epoch (int, optional):
				The current training epoch. Default: `None`.
		"""
		...
		if self.auto_insert_metric_name and metrics:
			...
			if value is not None:
				filename = f"{filename}_[{self.monitor}={value:0.4f}]"
