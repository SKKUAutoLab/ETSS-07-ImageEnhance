...
"""Operations that draw and visualize object on images.
"""
...
from torchkit.core.utils import to_4d_arraylist
from .imageproc import concatenate_images
from .imageproc import reshape_image
from .imageproc import to_pil_image
...
def show_images(
	...
	Args:
		images (Images):
			Images to be shown.
		nrow (int):
			Number of images displayed in each row of the grid. The final grid
			size is `[B / nrow, nrow]`.
		figure_num (int):
	...
	# NOTE: Convert to numpy and reshape
	cat_image = cat_image.numpy() if torch.is_tensor(cat_image) else cat_image
	cat_image = reshape_image(cat_image, False)
	
	# NOTE: Unnormalize image
	cat_image = unnormalize_image(cat_image)
...
def imshow_plt(
	...
	Args:
		...
		labels (Sequence[str], optional):
			The sequence of images' labels string.
		scale (int):
			Scale the size of matplotlib figure. `1` means (default size x 1).
		save_cfg (dict, optional):
			Save figure config.
		show (bool):
			If `True`, show the debug image, else skip. Default: `True`.
		show_max_n (int):
			Show at max n images. Default: `8`.
		wait_time (float):
			Wait some time (in seconds) to display the figure then reset.
		figure_num (int):
			The matplotlib figure id. Default: `0`.
	...
	"""
	# NOTE: Prepare images
	images_ = to_4d_arraylist(images)  # List of 4D-array
	images_ = [reshape_image(i, False) for i in images_]
	images_ = [unnormalize_image(i)    for i in images_]
...
def imshow_cls_plt(
	...
	Args:
		...
		labels (Sequence[str], optional):
			The sequence of images' labels string. Default: `None`.
		classlabels (ClassLabels, optional):
			The `ClassLabels` objects that contains all class labels in the
	...
	"""
	# NOTE: Prepare images
	images_ = images  # to_4d_arraylist(images)  # List of 4D-array
	images_ = reshape_image(images_, False)
	images_ = unnormalize_image(images_)
	images_ = images_[:show_max_n]
	
	# NOTE: Prepare preds and gts
	...
	if classlabels:
		pred_labels   = [[classlabels.get_name(value=l) for l in pred]
						 for pred in pred_labels]
		pred_1_labels = [classlabels.get_name(value=l) for l in pred_1_labels]
		gts 		  = [classlabels.get_name(value=l) for l in gts]
	
	# NOTE: Prepare labels
	if labels is None:
		labels = [f"" for i in range(images_.shape[0])]
	labels = [f"{l} \n gt={gt}"
			  for (l, pred, gt) in zip(labels, pred_1_labels, gts)]
	colors = ["darkgreen" if pred == gt else "red"
			  for (pred, gt) in zip(pred_1_labels, gts)]
	assert len(labels) == images_.shape[0]
