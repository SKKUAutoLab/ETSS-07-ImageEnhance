...
from .builder import BOTTLENECK_LAYERS
...
class Bottleneck(nn.Module):
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        y_hat = ((x + self.conv2(self.conv1(x))) if self.add
...
class BottleneckCSP(nn.Module):
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        y2    = self.conv2(x)
        y_hat = self.conv4(self.act(self.bn(torch.cat((y1, y2), dim=1))))
        return y_hat
...
class VoVCSP(nn.Module):
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        x1    = self.conv1(x1)
        x2    = self.conv2(x1)
        y_hat = self.conv3(torch.cat((x1, x2), dim=1))
        return y_hat
