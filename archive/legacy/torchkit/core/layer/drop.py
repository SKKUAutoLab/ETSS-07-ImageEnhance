...
from .builder import DROP_LAYERS
...
@DROP_LAYERS.register(name="drop_block_2d")
...
@DROP_LAYERS.register(name="dropout")
class Dropout(nn.Dropout):
    """A wrapper for ``torch.nn.Dropout``, We rename the ``p`` of `torch.nn.Dropout` to `drop_prob` so as to be
    consistent with `DropPath`
    """
