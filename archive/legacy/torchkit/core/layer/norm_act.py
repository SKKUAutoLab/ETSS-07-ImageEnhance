...
from .builder import NORM_ACT_LAYERS
...
@NORM_ACT_LAYERS.register(name="bn_act_2d")
@NORM_ACT_LAYERS.register(name="batch_norm_act_2d")
...
@NORM_ACT_LAYERS.register(name="bn_relu_2d")
@NORM_ACT_LAYERS.register(name="batch_norm_relu_2d")
...
class GroupNormAct(nn.GroupNorm):
    ...
    This module performs GroupNorm + Activation in a manner that will remain backwards compatible with weights trained
    with separate gn, act. This is why we inherit from GN instead of composing it as a .gn member.
    """
...
def convert_norm_act(norm_layer: FuncCls, act_layer: FuncCls) -> nn.Module:
    assert isinstance(norm_layer, FuncCls)
    assert act_layer is None or isinstance(act_layer, FuncCls)
    norm_act_kwargs = {}
