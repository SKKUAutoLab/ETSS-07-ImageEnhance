...
checkpoint = {
	...
	# `False`, then the check runs at the end of the validation. If `None`
...
inference = {
	...
	# run-specific subdirectory name, otherwise `version_${version}` is used.
	"shape": [256, 256, 3],
	# The input and output shape of the image as [H, W, C]. If `None`, use the
	# input image shape.
	"batch_size": 1,
...
data = {
	...
	#   most over-exposure. `*` is all images. Default: `1`.
	"shape": [256, 256, 3],
