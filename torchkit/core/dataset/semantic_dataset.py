...
"""The template for datasets used in semantic segmentation task.
"""
...
from torchkit.core.image import create_semantic_image
...
# MARK: - SegmentationDataset

class SemanticDataset(VisionDataset, metaclass=ABCMeta):
    """Semantic dataset is a base class that defines the pattern for all
    ...
    def __getitem__(self, index: int) -> Any:
        ...
        """
        items  = self.label_formatter.get_semantic_item(index=index)
        image  = items[0]
        target = items[1]
        rest   = items[2:]
        ...
        if os.path.isfile(cached_label_path):
            ...
            if self.caching_labels:  # Force re-cache
                cache = self.cache_labels(path=cached_label_path)  # Re-cache
                                           ...
                                           self.image_paths +
                                           self.semantic_paths):  # Dataset changed
                cache = self.cache_labels(path=cached_label_path)  # Re-cache
        else:
    ...
    def cache_semantic_images(self):
        ...
        n    = len(self.semantic_paths)
        gb   = 0  # Gigabytes of cached images
        pbar = tqdm(range(n), desc="Caching semantic segmentation image")
        for i in pbar:  # Should be max 10k images
            # image, hw_original, hw_resized
    ...
    def load_semantic_image(self, index: int) -> tuple[np.ndarray, ImageInfo]:
        ...
        if image is None:  # Not cached
            ...
            image = cv2.imread(path)  # BGR
            assert image is not None, (f"Semantic segmentation image not "
                                       f"found at: {path}.")
            ...
            return self.data[index].semantic, self.data[index].semantic_info
    
    # noinspection PyUnboundLocalVariable
    def load_mosaic(self, index: int) -> tuple[np.ndarray, np.ndarray]:
        """Load 4 images and semantic segmentation images and create a mosaic.
        ...
        Returns:
            image4 (np.ndarray):
                The mosaic image.
            semantic4 (np.ndarray):
                The mosaic semantic segmentation image.
        """
        shape         = self.shape
        ...
        # NOTE: Create mosaic image and enhancement
        for i, index in enumerate(indices):
            ...
            if i == 0:  # Top left
                ...
                semantic4 = np.full((yc * 2, xc * 2, semantic.shape[2]), 114, np.uint8)  # base image with 4 tiles
            ...
            semantic4[y1a:y2a, x1a:x2a] = semantic[y1b:y2b, x1b:x2b]    # image4[ymin:ymax, xmin:xmax]
            padw = x1a - x1b
            padh = y1a - y1b

        # NOTE: Create mosaic enhancement
    
        # NOTE: Augment
        image4, semantic4 = random_perspective_mask(
            image       = image4,
            mask        = semantic4,
    ...
    def write_custom_labels(self):
        ...
        for i in tqdm(range(len(self.data)),
                      desc="Scanning semantic segmentation images"):
            # image, hw_original, hw_resized
    ...
    def write_semantic_images(self):
        ...
        """
        for i in tqdm(range(len(self.semantic_paths)),
					  desc="Writing semantic segmentation images"):
            data          = self.data[i]
            semantic_path = self.semantic_paths[i]
