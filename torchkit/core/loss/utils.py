...
# The `torchkit.models.image_classification.losses.utils` defines some
...
def weighted_loss(loss_func: Callable):
	...
	The function only needs to compute element-wise loss without any
	reduction. This decorator will add weight and reduction arguments to the
	function. The decorated function will have the signature like
...
def convert_to_one_hot(y: torch.Tensor, num_classes: int) -> torch.Tensor:
	"""This function converts target class indices to one-hot vectors, given
	the number of classes.
	
	Args:
		y (torch.Tensor):
			The ground truth label of the prediction with shape (N, 1).
		num_classes (int):
			The number of classes.
	...
	Returns:
		one_hot_targets (torch.Tensor):
			Processed loss values.
	"""
	assert (torch.max(y).item() < num_classes), \
		"Class Index must be less than number of classes."
	one_hot_targets = torch.zeros(
		(y.shape[0], num_classes), dtype=torch.long, device=y.device
	)
	one_hot_targets.scatter_(1, y.long(), 1)
