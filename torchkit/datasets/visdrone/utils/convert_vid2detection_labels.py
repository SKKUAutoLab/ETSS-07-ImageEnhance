# ==================================================================== #
# File name: convert_vid2detection_labels.py
# Author: Long H. Pham
# Date created: 06/22/2021
# The `torchkit.datasets.visdrone.convert_labels_yolo` implements the
# operations to convert the bounding box labels to yolo format.
# ==================================================================== #
...
from torchkit.core.fileio import list_subdirs
...
Where:
	...
	<score>          : The score in the DETECTION result file indicates the confidence of the predicted bounding box
					   enclosing an object instance.The score in GROUNDTRUTH file is set to 1 or 0. 1 indicates the bounding
					   box is considered in evaluation, while 0 indicates the bounding box will be ignored.
	<object_category>: The object category indicates the type of annotated object, (i.e., ignored regions (0),
					   pedestrian (1), people (2), bicycle (3), car (4), van (5), truck (6), tricycle (7),
					   awning-tricycle (8), bus (9), motor (10), others (11))
	<truncation>     : The score in the DETECTION result file should be set to the constant -1. The score in the
	                   GROUNDTRUTH file indicates the degree of object parts appears outside a frame
	                   (i.e., no truncation = 0 (truncation ratio 0%), and partial truncation = 1(truncation ratio 1% ∼ 50%)).
	<occlusion>      : The score in the DETECTION result file should be set to the constant -1. The score in the
...
def convert_vid2detection_labels(root_dir: str, format: str = "yolo"):
	...
	Args:
		root_dir (str):
			The root directory that contains `train/val/testdev/testchallenge` directories.
		format (str):
			The format to convert the labels to.
			Default: `yolo`.
	"""
...
def _convert_labels_yolo(sequence_dir: str, labels_file: str):
	...
	Args:
		sequence_dir (str):
			The sequence directory that contains several images corresponding to the labels.
		labels_file (str):
			The `.txt` file that contains labels.
	"""
	with open(labels_file, "r") as file_in:
		...
		for i, index in enumerate(frame_indexes):
			...
			boxes        = bbox_xywh_cxcywh_norm(xywh=boxes, width=w, height=h)
...
def merge_labels(root_dir: str, format: str = "yolo", output_dir: str = "", shuffle: bool = False):
	...
	Args:
		...
		format (str):
			...
			Default: `yolo`.
		output_dir (str):
			The location to move the images and labels files to.
...
if __name__ == "__main__":
	...
	parser.add_argument("--data_dir",   type=str,  default="",     help="The path to the dataset directory that contains train/val/testdev/testchallenge directories.")
	parser.add_argument("--format",     type=str,  default="yolo", help="The format to convert the labels to.")
	parser.add_argument("--merge",      type=bool, default=True,   help="Should move all images and labels to a single location.")
	parser.add_argument("--output_dir", type=str,  default="",     help="The location to move the images and labels files to.")
