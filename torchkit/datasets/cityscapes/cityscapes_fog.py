...
__all__ = ["CityscapesFog", "CityscapesFogDataModule"]
...
data = {
    ...
    # The dataset's name.
    "subset": "foggy",
    # The sub-dataset name. One of: [`foggy`, `foggyDBF`]. Default: `foggy`.
    "extra": False,
    ...
    # One of: [0.005, 0.01, 0.02]. Default: `0.01`.
    "shape": [1024, 2048, 3],
...
@DATASETS.register(name="cityscapes_fog")
class CityscapesFog(EnhancementDataset):
    """The Cityscapes Fog dataset consists of multiple sub-datasets related to
    ...
    Attributes:
        root (str):
			...
			subdirectories.
        subset (str):
            The sub-dataset name. One of: ["foggy", "foggyDBF"].
            Default: `foggy`.
        extra (bool):
    ...
    def list_files(self):
        ...
        """
        logger.info(f"Cityscapes Foggy dataset only supports `split`: "
                    f"`train`, `val`, or `test`. Get: {self.split}.")
        
        # NOTE: List all image files
        image_patterns = [
            os.path.join(
                self.root, f"leftImg8bit_{self.subset}", self.split, "*",
                f"*_{beta}.png"
            )
        ...
        for image_path in image_paths:
            ...
            eimage_path = eimage_path.replace(
                f"leftImg8bit_{self.subset}", "leftImg8bit"
            )
            label_path  = image_path.replace(
                f"leftImg8bit_{self.subset}",
                f"leftImg8bit_{self.subset}_customs"
            )
